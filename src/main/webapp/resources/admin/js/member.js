...
function goToDetails(cust_No, cust_Id, cust_Name, cust_Birth, show) {
    ...
    // 세부 정보를 전달할 URL 생성
    var url = "/dreams_project2/admin_memberDetail.cc?cust_No=" + encodeURIComponent(cust_No) +
...
function enable_cust(custNo) {
	if(confirm("계정을 복구하시겠습니까?")) {
		window.location.href = "/dreams_project2/admin_enable_cust.cc?cust_No=" + custNo;
	}
	else {
	}
}
...
function block_cust(custNo) {
	if(confirm("계정을 삭제하시겠습니까?")) {
		window.location.href = "/dreams_project2/admin_block_cust.cc?cust_No=" + custNo;
	}
	else {
	}
}
...
function delete_cust(custNo) {
	if(confirm("정말로 이 회원을 영구적으로 삭제하시겠습니까?")) {
		window.location.href = "/dreams_project2/admin_delete_cust.cc?cust_No=" + custNo;
	}
	else {
	}
}